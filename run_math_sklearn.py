...
# [NEW] 你的 sklearn 模型字典（用占位 SEED，运行时再用 args.seed 替换）
def build_sklearn_models(SEED: int):
    return {
        "SVM_RBF": Pipeline([
            ('scaler', StandardScaler()),
            ('svm_rbf', SVC(kernel='rbf', C=1.0, random_state=SEED, probability=False))
        ]),
        "SVM_Linear": Pipeline([
            ('scaler', StandardScaler()),
            ('svm_linear', SVC(kernel='linear', C=1.0, random_state=SEED, probability=False))
        ]),
        "SVM_Poly": Pipeline([
            ('scaler', StandardScaler()),
        ...
        "SVM_Sigmoid": Pipeline([
            ('scaler', StandardScaler()),
            ('svm_sigmoid', SVC(kernel='sigmoid', C=1.0, random_state=SEED))
        ]),
        "SVM_L1": Pipeline([
            ('scaler', StandardScaler()),
        ...
        "SVM_ElasticNet": Pipeline([
            ('scaler', StandardScaler()),
            ('svm_elastic', LogisticRegression(
                penalty="elasticnet", solver="saga",
                l1_ratio=0.5, max_iter=2000, random_state=SEED
            ))
        ]),
        "LASSO_then_SVM_RBF": Pipeline([
            ('scaler_before', StandardScaler()),
            ('lasso_select', SelectFromModel(
                estimator=LassoCV(cv=5, max_iter=50000, tol=1e-3, n_jobs=1, random_state=SEED),
                threshold="median"
            )),
            ('scaler_after', StandardScaler(with_mean=True)),
            ('svm_rbf', SVC(kernel='rbf', C=1.0, random_state=SEED, probability=False))
        ]),
        "LASSO_then_SVM_Linear": Pipeline([
            ('scaler_before', StandardScaler()),
            ('lasso_select', SelectFromModel(
                estimator=LassoCV(cv=5, max_iter=50000, tol=1e-3, n_jobs=1, random_state=SEED),
                threshold="median"
        ...
        "LogisticRegression": Pipeline([
            ('scaler', StandardScaler()),
            ('logreg', LogisticRegression(random_state=SEED, solver='liblinear', max_iter=200))
        ]),
        "KNeighbors": Pipeline([
            ('scaler', StandardScaler()),
        ...
        "RandomForest": RandomForestClassifier(n_estimators=100, random_state=SEED, max_depth=10),
...
# [NEW] 将 [B, C, L] 转成 [B, C, A, 200]
def _reshape_to_patches(x: torch.Tensor, patch_size: int = 200):
    # x: [B, C, L] ；L 已保证是 patch_size 的整数倍
    B, C, L = x.shape
    A = L // patch_size
    return x.view(B, C, A, patch_size)

# [NEW] 从 DataLoader 提取特征
def _extract_features(model, loader, device, patch_size=200):
    model.eval()
    feats, labels = [], []
    with torch.no_grad():
        for xb, yb in loader:
            # xb: [B, C, L]
            xb = _reshape_to_patches(xb, patch_size).to(device, non_blocking=True)
            fb = model(xb)                  # head 已经是 Identity -> [B, embed_dim]
            feats.append(fb.cpu().numpy())
            labels.extend(np.asarray(yb))
    if len(feats) == 0:
        return np.empty((0,)), np.asarray(labels)
    return np.concatenate(feats, axis=0), np.asarray(labels)

# [NEW] 训练并评估若干 sklearn 模型
def _run_sklearn(Xtr, ytr, Xval, yval, Xte, yte, model_keys, seed, is_binary, metrics_list):
    models = build_sklearn_models(seed)
    select_keys = [k.strip() for k in model_keys.split(',') if k.strip() in models]
    results = {}
    for key in select_keys:
        ...
        for split_name, X, y in [('val', Xval, yval), ('test', Xte, yte)]:
            ...
            acc = metrics.accuracy_score(y, y_pred) if X.size else float('nan')
            bal = metrics.balanced_accuracy_score(y, y_pred) if X.size else float('nan')
            res = {'accuracy': acc, 'balanced_accuracy': bal}
            # 可选指标
            if not is_binary and 'f1_weighted' in metrics_list:
                res['f1_weighted'] = metrics.f1_score(y, y_pred, average='weighted') if X.size else float('nan')
            if is_binary:
                # 有些管线没有 predict_proba；此时跳过 AUC 类指标
                if hasattr(clf, 'predict_proba'):
                    p = clf.predict_proba(X)[:, 1]
                    res['roc_auc'] = metrics.roc_auc_score(y, p) if X.size else float('nan')
                    res['pr_auc']  = metrics.average_precision_score(y, p) if X.size else float('nan')
            results.setdefault(key, {})[split_name] = res
    return results
...
def get_args():
    ...
    # [NEW] 特征提取 + sklearn 评估开关
    parser.add_argument('--sklearn_eval', action='store_true', default=False,
                        help='Use pretrained model as a frozen feature extractor and evaluate with sklearn models.')
    parser.add_argument('--sk_models', type=str, default='SVM_RBF,SVM_Linear,LogisticRegression,RandomForest',
                        help='Comma-separated sklearn model keys to run. See sklearn_models_available.')
    parser.add_argument('--save_features', type=str, default='',
                        help='Optional path to save extracted features as NPZ (per fold).')
...
def _run_one_fold(args, ds_init, ch_names_upper, metrics, fold_id, subject_id,
    ...
    # [NEW] sklearn_eval：冻结为“特征提取器”（head 置 Identity），直接提特征 + 训练 sklearn
    if args.sklearn_eval:
        # 1) 置 head=Identity，确保 forward 输出 pooled features
        ...
        # 2) 提取三份特征
        Xtr, ytr = _extract_features(model, data_loader_train, device, patch_size=patch_size)
        Xval, yval = _extract_features(model, data_loader_val,   device, patch_size=patch_size) if data_loader_val is not None else (np.empty((0,)), np.empty((0,)))
        Xte, yte   = _extract_features(model, data_loader_test,  device, patch_size=patch_size) if data_loader_val is not None else (np.empty((0,)), np.empty((0,)))

        # 3) 可选：保存特征
        if args.save_features:
            os.makedirs(args.output_dir or '.', exist_ok=True)
            save_to = os.path.join(args.output_dir, f"S{subject_id}_F{fold_id}_feats.npz") if args.output_dir else args.save_features
            np.savez_compressed(save_to, Xtr=Xtr, ytr=ytr, Xval=Xval, yval=yval, Xte=Xte, yte=yte)
            print(f"[S{subject_id} F{fold_id}] Saved features -> {save_to}")

        # 4) 跑你挑的 sklearn 模型
        is_binary = (args.nb_classes == 1)
        sklearn_res = _run_sklearn(Xtr, ytr, Xval, yval, Xte, yte,
                                   model_keys=args.sk_models, seed=args.seed,
                                   is_binary=is_binary, metrics_list=metrics)

        # 5) 为了与你的原汇总逻辑对接：把“test 指标”灌回 best_* 字典（按每个模型各打一条 log）
        best_val_stats, best_test_stats = {}, {}
        for key, d in sklearn_res.items():
            v = d.get('val', {})
            t = d.get('test', {})
            print(f"[S{subject_id} F{fold_id}] sklearn[{key}]  Val: {v} | Test: {t}")
            # 记录一个代表性模型到 best_*（若你想都记，可以在 done.json/ckpt 里写多模型字段）
            if not best_val_stats:
                best_val_stats = v
                best_test_stats = t

        # 6) 写 done.json（与原训练分支保持一致）
        try:
            if args.output_dir and utils.is_main_process():
                ...
                with open(done_path, "w", encoding="utf-8") as f:
                    json.dump({
                        ...
                        "best_test": best_test_stats,
                        "sklearn_models": args.sk_models
